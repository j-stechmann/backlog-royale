...
| :--- | :--- | :--- |
| `room` | yes | Room name (= room ID). Creates the room on first join. Max 40 characters — longer names are rejected with HTTP 400 before the upgrade. |
...
- Missing `room` or `name`, or a `room` longer than 40 characters → HTTP 400 before upgrade.
