...
- Corrected two accuracy details surfaced by review: the log catalog now explains that a ghost client from a dropped unregister lingers until its outbound send buffer fills or the room self-destructs (the socket is already dead, so no read-pump cleanup follows), and the protocol reference no longer calls rate-limited messages "silently dropped" — the server logs a warning, only the client gets no feedback.
