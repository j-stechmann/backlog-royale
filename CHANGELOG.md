...
- The header controls now follow the app's button language ([#131](https://github.com/j-stechmann/backlog-royale/issues/131)): the "Become Dealer"/"Go AFK" toggles and the copy-invite button previously used `rounded-lg` with no press feedback, no shadow, and a gray fill unlike every other button (`rounded-xl`, `active:scale-95`, bordered surface, `shadow-sm`). The toggles, copy-invite, and burger buttons are now outlined surface buttons matching the player-list's "Reveal Results" (`bg-surface` + `border-line`, hover to `border-accent`/`accent-text`, `rounded-xl`, `active:scale-95`), with active states keeping their tinted `*-soft` + `*/30` badge look, and the pill keeps its badge radius (`rounded-lg`). The shared `ThemeToggle` segmented control was re-chromed the same way (`rounded-xl` bordered surface, `rounded-lg` active segment; was `rounded-full` on gray fill) so it matches in both the header and the join screen. The toggles' hover text also switched to white, which breaks AA contrast in dark mode where `accent-strong`/`warn-strong` are *light* colors ([ADR 0009](docs/adr/0009-semantic-tokens-and-dark-theme.md)) — they now hover to `text-content-inverse`, correct in both themes. The Live/Reconnecting pill is now status-tinted like the other status badges: accent-soft with `border-accent/30` when connected, `bg-danger text-white` while reconnecting.
- The header no longer overflows horizontally on small vertical screens ([PR #132](https://github.com/j-stechmann/backlog-royale/pull/132)): it was a single fixed-height row with no truncation or wrapping, so the room ID (unbounded user input) plus the five control elements (~320 px) could not fit a 375 px phone and pushed the page wide. On screens below `sm` the controls now collapse into a burger menu (Menu/X icon) — an accessible dropdown (`aria-expanded`/`aria-controls`, closes on outside click, Escape, and after any action, with focus returned to the burger) with full-width, always-labeled rows for the AFK/Dealer toggles, the connection pill, the theme toggle, and copy-invite. The room ID truncates via `min-w-0`/`truncate`, and the room-name length is now enforced server-side (40 characters, HTTP 400 beyond that) so the join form's `maxLength` cap matches a real invariant instead of a frontend-only guard; the desktop (`sm:`) header is unchanged.
- No more layout shift when a round is revealed: the voting panel previously replaced the card grid with the (usually shorter) vote summary, so the panel, player list, and footer jumped upward on reveal and back on the next round. The voting panel now stacks its views (card grid, vote summary, dealer notice) in a single fixed-size panel shell with all layers in one CSS grid cell — the panel height is always the height of the tallest view, so nothing moves in any viewport, at any width. Views crossfade (300 ms, opacity + visibility so the outgoing view fades out instead of vanishing); hidden layers are kept non-interactive via `inert` and `aria-hidden`. The summary/dealer views are vertically centered inside the shell, without their own nested panel chrome — the shell is the panel. When the visible view changes (reveal, next round, dealer takeover), focus moves to that view's heading (`Voting Summary` / `You are the Dealer` / `Cast your vote`), so keyboard and screen-reader users keep their place instead of restarting from the top of the document; initial page load never steals focus. Also removed the summary's dead `animate-in` entrance classes (the `tw-animate-css` plugin was never installed, so they never compiled) and reserved scrollbar space via `scrollbar-gutter: stable` so shortening the page on reveal no longer causes a horizontal scrollbar jump on desktop.
- The footer (version label + Imprint/Privacy links) no longer scrolls over page content ([#126](https://github.com/j-stechmann/backlog-royale/issues/126)): it was `position: fixed` in the bottom-right corner, so cards and rows rendered underneath it while scrolling. It now sits in the normal document flow — every view (join screen, game, legal pages) is a flex column that pushes the footer to the viewport bottom on short pages and keeps it below the content on long ones. The wrappers use `min-h-dvh` (dynamic viewport height) instead of `min-h-screen`, so on mobile browsers with dynamic toolbars the footer stays within the visible viewport. Also fixed a stale "bottom-right corner" reference in the bug-reporting guide ([docs/CONTRIBUTING.md](docs/CONTRIBUTING.md)).
...
- Qualified the multi-tab coexistence guarantee, which was overstated: coexistence (no `prevId` sent) holds for tabs reaching a room via its URL (auto-join) or via reconnects, but `joinRoom` sets `prevIdToEvict` on any form-join, so a second tab that opens the plain URL and submits the join form still forwards the shared localStorage ID and evicts the first tab's live connection (which reconnects fresh). Documented in the FAQ, frontend/backend architecture pages, Features, ADR 0008 (now records the form-join sharp edge under Consequences), Development setup pitfalls, and Troubleshooting.
- Corrected the ping direction in the self-hosting guide and protocol reference: the **server** sends pings every 54 s and enforces the 60 s pong-refreshed read deadline; browsers only answer pongs and cannot originate WebSocket pings. The nginx `proxy_read_timeout` advice is unchanged.
